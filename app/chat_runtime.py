...
def chat(tenant_id: str, messages: list[dict[str, str]]) -> dict[str, Any]:
    ...
    if not api_key:
        return {
            ...
            "message": "NINA runtime is online, but conversational LLM access is not configured yet.",
        }
    ...
    append_record(
        tenant_id,
        "chat.completed",
        {"model": model, "message_count": len(messages), "response_digest": __import__("hashlib").sha256(text.encode()).hexdigest()},
    )
